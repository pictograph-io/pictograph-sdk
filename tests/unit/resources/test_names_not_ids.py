...
class TestImageSegmentResolvesAUuid:
    """B333: the per-image annotation/delete/review/split routes address
    ``{dataset}/{image_path}``, so a UUID must be resolved to its directory +
    filename. The resolver reads ``/images/{id}/metadata`` (JSON) - NOT
    ``/images/{id}``, which serves the raw image BYTES and made every per-image
    UUID call die on ``response.json()`` (UnicodeDecodeError on the magic bytes).
    A documented method raising on a documented input - the whole bug.
    """

    IMG = "8f92a5e9-a00e-44c7-9ed8-e618e2f71743"

    def test_a_uuid_reads_the_json_metadata_route_not_the_bytes_route(
        self, httpx_mock: HTTPXMock, transport: Transport
    ) -> None:
        from pictograph.resources._resolve import image_segment

        # ONLY the JSON metadata route is mocked. The old code hit /images/{id}
        # (bytes); that request is unmatched here, so it fails on it - the RED.
        httpx_mock.add_response(
            url=f"{BASE}/api/v1/developer/images/{self.IMG}/metadata",
            json={"data": {"directory_path": "/val", "filename": "000000546325.jpg"}},
        )
        assert image_segment(transport, self.IMG) == "val/000000546325.jpg"
        assert str(httpx_mock.get_requests()[0].url).endswith(f"/images/{self.IMG}/metadata")

    def test_a_root_image_folds_to_the_bare_filename(
        self, httpx_mock: HTTPXMock, transport: Transport
    ) -> None:
        from pictograph.resources._resolve import image_segment
        ...
        httpx_mock.add_response(
            url=f"{BASE}/api/v1/developer/images/{self.IMG}/metadata",
            json={"data": {"directory_path": "/", "filename": "photo.jpg"}},
        )
        assert image_segment(transport, self.IMG) == "photo.jpg"

    def test_a_metadata_row_without_a_filename_raises_not_found(
        self, httpx_mock: HTTPXMock, transport: Transport
    ) -> None:
        from pictograph.resources._resolve import image_segment
        ...
        with pytest.raises(NotFoundError):
            image_segment(transport, self.IMG)

    def test_a_filename_needs_no_request(self, transport: Transport) -> None:
        from pictograph.resources._resolve import image_segment

        # A filename (not a uuid) folds with the directory locally - no round-trip.
        assert image_segment(transport, "photo.jpg", directory_path="/train") == "train/photo.jpg"
