...
class TestImageSegmentResolvesAUuid:
    """Async twin of the sync B333 fix: a UUID resolves via the JSON
    ``/images/{id}/metadata`` route, not ``/images/{id}`` (raw bytes)."""

    IMG = "8f92a5e9-a00e-44c7-9ed8-e618e2f71743"
    _BASE = "https://api.test.local"

    async def test_a_uuid_reads_the_json_metadata_route(
        self, httpx_mock: HTTPXMock, transport: AsyncTransport
    ) -> None:
        # Pin the URL to /metadata so the old /images/{id} (bytes) call is
        # unmatched here and the test fails on it - the RED against the old code.
        httpx_mock.add_response(
            method="GET",
            url=f"{self._BASE}/api/v1/developer/images/{self.IMG}/metadata",
            json={"data": {"directory_path": "/val", "filename": "x.jpg"}},
        )
        assert await _resolve.image_segment(transport, self.IMG) == "val/x.jpg"

    async def test_a_metadata_row_without_a_filename_raises(
        self, httpx_mock: HTTPXMock, transport: AsyncTransport
    ) -> None:
        httpx_mock.add_response(
            method="GET",
        ...
        with pytest.raises(NotFoundError):
            await _resolve.image_segment(transport, self.IMG)
