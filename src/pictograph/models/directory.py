...
class Directory(BaseModel):
    ...
    #: Images DIRECTLY in this directory (exact-path match).
    image_count: int = 0
    #: Images in this directory's WHOLE subtree (itself + every descendant) - the
    #: recursive rollup the grid tile badge shows. 0 on an older backend.
...
class DirectoryTreeNode(BaseModel):
    ...
    #: Images DIRECTLY in this directory (exact-path match).
    image_count: int = 0
    #: Images in this directory's WHOLE subtree (itself + every descendant).
    nested_image_count: int = 0
