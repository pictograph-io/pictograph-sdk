...
class AnnotationComments(Resource):
    ...
    def react(self, comment_id: str, reaction: str) -> AnnotationComment:
        """Add the caller's reaction to a comment (idempotent). ``reaction`` must
        be one of :attr:`REACTIONS`."""
        response = self._transport.request("PUT", f"{_API_PATH}/{comment_id}/reactions/{reaction}")
        return self._parse(AnnotationComment, response["comment"])

    def unreact(self, comment_id: str, reaction: str) -> AnnotationComment:
