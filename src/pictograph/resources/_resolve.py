...
def image_segment(
    ...
    *,
    ...
    """
    if looks_like_id(image):
        # `/images/{id}` serves the raw BYTES (the exports image_url contract),
        # so parsing it as JSON raised UnicodeDecodeError on the image's magic
        # bytes - every per-image call with a UUID was broken. `/{id}/metadata`
        # is the by-id JSON twin of `/{dataset}/{image_path}/metadata` and hands
        # back the directory + filename this segment folds together.
        row = transport.request("GET", f"/api/v1/developer/images/{image}/metadata")
        ...
        if not filename:
            ...
            raise NotFoundError(f"No image with id {image!r} in your organization.")
