"""Webhooks resource - register outbound sinks + inspect / replay deliveries.

Register an https endpoint, then Pictograph POSTs signed events to it (e.g. when a
workflow run finishes). Deliveries are durable and retried; inspect them with
:meth:`Webhooks.deliveries` and re-queue a failed one with :meth:`Webhooks.replay`.
"""
...
class Webhooks(Resource):
    """Manage outbound webhook endpoints and their delivery log."""
    ...
    def create(
        ...
        *,
        ...
        ``url`` must be https and publicly routable (private / loopback / cloud-metadata
        targets are rejected). Verify deliveries with the returned secret.

        ``auth_headers`` are optional custom request headers (e.g.
        ``{"Authorization": "Bearer …"}``) sent on every delivery, so the endpoint can
        sit behind your own gateway - complementary to the signature, not a replacement.
        They are stored encrypted and never returned by :meth:`list` / :meth:`get`
        (only their names are, as ``auth_header_names``).
        """
        ...
        response = self._transport.request("POST", _ENDPOINTS, json=body)
    ...
    def event_types(self) -> list[str]:
        ...
        response = self._transport.request("GET", _EVENT_TYPES)
    ...
    def list(self) -> Sequence[WebhookEndpoint]:
        ...
        response = self._transport.request("GET", _ENDPOINTS)
        return self._parse_list(WebhookEndpoint, response.get("endpoints", []))

    def get(self, endpoint: str) -> WebhookEndpoint:
        """Fetch a single endpoint by its registered URL (an id also works)."""
        endpoint_id = _resolve.webhook_endpoint_id(self._transport, endpoint)
        response = self._transport.request("GET", f"{_ENDPOINTS}/{endpoint_id}")
    ...
    def update(
        ...
        *,
        ...
        """Update an endpoint's ``url`` / ``description`` / ``event_types`` /
        ``enabled`` state (only the fields you pass change). Re-enabling a
        disabled endpoint clears its failure health state.

        ``auth_headers`` replaces the endpoint's custom headers wholesale; pass ``{}``
        to clear them. Omit it to leave them unchanged.
        """
        body: dict[str, Any] = {}
        if url is not None:
            body["url"] = url
        ...
        endpoint_id = _resolve.webhook_endpoint_id(self._transport, endpoint)
        response = self._transport.request("PATCH", f"{_ENDPOINTS}/{endpoint_id}", json=body)
        return self._parse(WebhookEndpoint, response["endpoint"])

    def delete(self, endpoint: str) -> None:
        """Delete an endpoint, by its registered URL, and its delivery history."""
        endpoint_id = _resolve.webhook_endpoint_id(self._transport, endpoint)
        self._transport.request("DELETE", f"{_ENDPOINTS}/{endpoint_id}")

    def test(self, endpoint: str) -> dict[str, Any]:
        """Send a synthetic signed test event; returns the immediate delivery result."""
        endpoint_id = _resolve.webhook_endpoint_id(self._transport, endpoint)
        result: dict[str, Any] = self._transport.request("POST", f"{_ENDPOINTS}/{endpoint_id}/test")
        return result

    def rotate_secret(self, endpoint: str) -> CreatedWebhookEndpoint:
        ...
        endpoint_id = _resolve.webhook_endpoint_id(self._transport, endpoint)
        response = self._transport.request("POST", f"{_ENDPOINTS}/{endpoint_id}/rotate-secret")
        return self._parse(CreatedWebhookEndpoint, response)
    ...
    def deliveries(
        ...
        *,
        ...
        if endpoint is not None:
            params["endpoint_id"] = _resolve.webhook_endpoint_id(self._transport, endpoint)
        if status is not None:
            params["status"] = status
        response = self._transport.request("GET", _DELIVERIES, params=params)
    ...
    def replay(self, delivery_id: str) -> None:
        ...
        self._transport.request("POST", f"{_DELIVERIES}/{delivery_id}/replay")
