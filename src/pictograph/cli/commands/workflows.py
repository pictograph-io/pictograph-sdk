...
def update_workflow(
    ...
    graph: Annotated[
        Path | None,
        typer.Option("--graph", "-g", help="Path to a NEW graph JSON file ({version,nodes,edges}); replaces the graph."),
    ] = None,
    name: Annotated[str | None, typer.Option("--name")] = None,
    readme: Annotated[str | None, typer.Option("--readme", help="Markdown card shown in the app.")] = None,
    description: Annotated[str | None, typer.Option("--description")] = None,
    status: Annotated[str | None, typer.Option("--status", help="active / archived.")] = None,
    api_key: Annotated[str | None, typer.Option("--api-key")] = None,
) -> None:
    graph_dict = None
    if graph is not None:
        if not graph.is_file():
            raise typer.BadParameter(f"graph file not found: {graph}", param_hint="--graph")
        graph_dict = json_module.loads(graph.read_text(encoding="utf-8"))
    if graph_dict is None and name is None and readme is None and description is None and status is None:
        raise typer.BadParameter("nothing to update: pass at least one of --graph/--name/--readme/--description/--status")
    client = get_client(api_key)
    wf = client.workflows.update(
        workflow, name=name, readme=readme, description=description, graph=graph_dict, status=status,  # type: ignore[arg-type]
    )
