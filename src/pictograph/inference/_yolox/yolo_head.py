...
# (https://github.com/Megvii-BaseDetection/YOLOX), path yolox/models/yolo_head.py
# MODIFIED (Apache-2.0 § 4b): the `yolox.utils` and `loguru` imports are
# redirected onto the vendored `._upstream_utils`, which carries those four
# functions verbatim and binds `logger` to the stdlib; and the four `.type(<legacy
# type string>)` casts are expressed as `.to(device=, dtype=)`, which upstream's
# own sibling call (`get_losses(..., dtype=xin[0].dtype)`) already uses, so the
# head runs on devices that have no legacy tensor type. Nothing else changed.
# See ./NOTICE.
# ------------------------------------------------------------------------
...
from ._upstream_utils import bboxes_iou, cxcywh2xyxy, logger, meshgrid, visualize_assign
