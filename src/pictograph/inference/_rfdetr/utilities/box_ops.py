...
# PICTOGRAPH: the `torch.jit.script`-compiled twin of the loss above is dropped.
# It is consumed only by `models/matcher.py`, which is a TRAINING module and is
# not vendored - so it had no caller here, yet compiled on import and emitted a
# `torch.jit.script is deprecated` warning on torch >= 2.12 in the user's process.
