...
from pictograph.inference._rfdetr.assets.model_weights import download_pretrain_weights, validate_pretrain_weights
...
from pictograph.inference._rfdetr.utilities.state_dict import _ckpt_args_get, remap_projector_to_cross_attn, validate_checkpoint_compatibility
...
# PICTOGRAPH: upstream's `@deprecated` wrapper (pyDeprecate) is dropped - it only
# warned about the `train_config` argument, which no caller here passes.
def load_pretrain_weights(
    ...
    try:
        checkpoint = safe_torch_load(pretrain_weights)
    except Exception:
...
def apply_lora(nn_model: torch.nn.Module) -> None:
    ...
    Raises:
        ImportError: If ``peft`` is not installed (``pip install peft``).
    """
    # PICTOGRAPH: upstream's message here told the user to `pip install "rfdetr[lora]"`.
    # The SDK never installs rfdetr, and this function is unreachable on a reload
    # (LoRA is a TRAINING adapter; `model_config.backbone_lora` is False for every
    # checkpoint we publish), so the hint is replaced with one that names the real
    # requirement.
    try:
        from peft import LoraConfig, get_peft_model
    except ImportError as exc:
        raise ImportError(
            "Applying a LoRA adapter needs the 'peft' package:\n"
            "    pip install peft"
        ) from exc
