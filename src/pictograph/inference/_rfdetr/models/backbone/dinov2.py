...
class DinoV2(nn.Module):
    def __init__(
        ...
        if not use_windowed_attn:
            ...
            # PICTOGRAPH: this branch pulls a DINOv2 backbone from the HuggingFace
            # hub, which only happens when TRAINING from scratch-ish weights. A
            # checkpoint reload always takes the windowed branch below, so the
            # import is deferred rather than made a package dependency.
            from transformers import AutoBackbone  # noqa: PLC0415
