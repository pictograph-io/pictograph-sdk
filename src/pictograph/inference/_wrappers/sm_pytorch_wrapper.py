...
class SemanticSegmentationModelPyTorch:
    def __init__(
        ...
        self.mask_max_side: int | None = mask_max_side
    ...
    def predict(
        ...
        if postprocess:
            # Resize the raw output to the CONTOUR resolution: the full frame by default,
            # or a bounded size when mask_max_side is set (workflow 4K perf) - the masks +
            # their polygons are then produced at that resolution and the caller scales the
            # polygons back to full frame. Only ever downscales below the frame, so a
            # sub-cap image is byte-identical.
            target_w, target_h = bounded_mask_size(
                width_orig, height_orig, getattr(self, "mask_max_side", None)
            )
            ...
            if onnx_pred.ndim == 3:
                ...
                for i in range(onnx_pred.shape[2]):
                    channel = cv2.resize(
                        onnx_pred[:, :, i], (target_w, target_h), interpolation=cv2.INTER_LINEAR
                    )
                ...
                onnx_pred = cv2.resize(
                    onnx_pred, (target_w, target_h), interpolation=cv2.INTER_LINEAR
                )
