...
from .mask_to_polygon import bounded_mask_size
...
def decode_segmentation_outputs(
    ...
    *,
    ...
    mask_max_side: int | None = None,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """RF-DETR segmentation outputs → (boxes_xyxy, scores, class_ids, masks).

    ``mask_max_side`` (workflow 4K perf, 2026-09-13): when set, each instance mask is
    resized to at most this many pixels on its longest side instead of to the full
    original frame, so the downstream mask->polygon runs on a fraction of the pixels.
    ``masks`` then come back at that reduced resolution; the caller lifts the polygons
    back to full frame via the mask/frame ratio. None (default) keeps full-resolution
    masks - byte-identical, so deployment / batch / SDK are unchanged.
    ...
    **The one reduction both engines share.** Same shape as the detection twin
    ...
    # Mask contour resolution: bounded when mask_max_side is set (workflow 4K perf),
    # else the full frame (unchanged). Polygons are lifted back to frame px by the caller.
    mask_w, mask_h = bounded_mask_size(orig_w, orig_h, mask_max_side)
    ...
    empty = (
        ...
        np.zeros((0, mask_h, mask_w), dtype=np.uint8),
    )
    ...
    # Process masks: resize the model-resolution mask logits to the CONTOUR resolution
    # (the full frame by default, or the bounded size when mask_max_side is set), then
    # threshold. Contouring + polygon extraction then run at this resolution; the caller
    # scales the resulting polygons back to full frame.
    if masks_logits is not None and len(masks_logits) > 0:
        processed_masks = []
        for mask in masks_logits:
            mask_resized = cv2.resize(mask, (mask_w, mask_h), interpolation=cv2.INTER_LINEAR)
        ...
        masks = np.zeros((len(boxes_xyxy), mask_h, mask_w), dtype=np.uint8)
...
class RFDETRSegDetector:
    ...
    def __init__(
        ...
        # Bounded mask-contour resolution (workflow 4K perf); None = full frame (default).
    ...
    def postprocess(
        ...
        return decode_segmentation_outputs(
            ...
            # getattr default: an instance built via __new__ (test stubs, external callers)
            # or before this field existed has no attr -> None -> full-res, old behavior.
            mask_max_side=getattr(self, "mask_max_side", None),
        )
