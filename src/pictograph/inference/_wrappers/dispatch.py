...
def build_wrapper(
    *,
    ...
    seg_mask_max_side: int | None = None,
):
    """Construct the correct ONNX wrapper for a model. Mirrors the batch service.

    ``seg_mask_max_side`` (workflow 4K perf, 2026-09-13): caps the resolution at which
    a segmentation model's masks are contoured into polygons (see the seg wrappers).
    None (default) = full frame, so every non-workflow caller is byte-identical; the
    workflow runner passes a bounded value for a large speedup on 4K video.
    ...
    if model_type == "instance_segmentation":
        return RFDETRSegDetector(
            ...
            providers=providers,
            sess_options=sess_options,
            mask_max_side=seg_mask_max_side,
        )
...
def _mask_scale_for(masks, orig_h: int, orig_w: int) -> tuple:
    """(sx, sy) that lifts a polygon traced on ``masks`` back to full-frame pixels.

    (1, 1) when the masks are already at frame resolution (the default seg path) OR
    empty - so the fast bounded-resolution path (masks smaller than the frame) is the
    only case that scales, and every full-resolution caller is byte-identical."""
    if masks is None or len(masks) == 0:
        return (1.0, 1.0)
    mh, mw = int(masks.shape[1]), int(masks.shape[2])
    if mh == orig_h and mw == orig_w:
        return (1.0, 1.0)
    return (orig_w / float(mw), orig_h / float(mh))
...
def _instance_seg_to_annotations(
    ...
    for i in range(len(boxes)):
        ...
        if output_type != "bbox" and mask_i is not None:
            try:
                poly_body = mask_to_all_polygons(mask_i, scale=mask_scale)
            except Exception:
                poly_body = None
...
def _semantic_seg_to_annotations(
    masks, classes, class_filter, prob_maps=None, mask_scale=(1.0, 1.0)
) -> list[dict[str, Any]]:
    ...
    for cid, mask in enumerate(masks if masks is not None else []):
        ...
        if present is not None and not present[cid]:
            ...
            continue
        try:
            poly_bodies = mask_to_instance_polygons(mask, scale=mask_scale)
        except Exception:
            poly_bodies = []
...
def infer_image(
    ...
    *,
    ...
    if model_type == "instance_segmentation":  # rfdetr-seg
        ...
        _oh, _ow = img_bgr.shape[:2]
        _scale = _mask_scale_for(masks, _oh, _ow)
        return {
            "model_type": model_type,
    ...
    if model_type == "semantic_segmentation":
        ...
        return {
            "model_type": model_type,
            "predictions": _semantic_seg_to_annotations(
                masks, classes, class_filter, prob_maps, mask_scale=_scale
            ),
        }
