...
``epsilon = min(0.0036 * perimeter, 0.00216 * image_diagonal)``) so auto-annotate
polygons produced by user-trained models land in the exact same shape as those
produced by SAM3. The 0.0036 coefficient is a backend LOCKSTEP constant (retuned
from 0.002 on 2026-08-28) grep-asserted by ``modal/tests/test_sahi_utils.py``.
"""
...
# ── Bounded-resolution contouring (workflow 4K perf, 2026-09-13) ──────────────
# The dominant cost of a segmentation workflow over 4K video was resizing EVERY
# instance mask to full frame (8.3 MP) and running findContours there, ~42x/frame.
# But the emitted polygon is Douglas-Peucker simplified to ~0.00216 * image_diagonal
# (~9.5 px at 4K), so tracing the contour at full 4K is wasted precision. Contour at
# a CAPPED resolution and scale the vertices up: because both approxPolyDP epsilon
# terms scale linearly with resolution, the scaled-up polygon has the SAME simplification
# granularity as one traced at full res - visually identical after simplification - at a
# fraction of the pixel work. A frame at or below the cap is untouched (scale == identity),
# so smaller images (deployment / batch / SDK) stay byte-identical.
SEG_CONTOUR_MAX_SIDE = 1024


def bounded_mask_size(orig_w: int, orig_h: int, max_side: int | None) -> tuple[int, int]:
    """The (w, h) to contour a mask at so its longest side <= ``max_side`` (aspect
    preserved), or (orig_w, orig_h) when ``max_side`` is falsy or the frame already
    fits. Only ever downscales - a small image is returned unchanged, so the scaled
    polygon path is a no-op there."""
    if not max_side:
    ...
    return (max(1, int(round(orig_w * s))), max(1, int(round(orig_h * s))))
...
def mask_to_polygon(
    mask: np.ndarray, scale: tuple[float, float] = (1.0, 1.0)
) -> list[dict[str, float]]:
    ...
    ``scale`` = (sx, sy) multiplies the output coordinates - used when the mask was
    contoured at a reduced resolution and the polygon must land in full-frame pixels
    (default (1, 1) = no scaling, byte-identical to before).
    """
    ...
    if len(simplified) < 3:
        ...
        return []
    sx, sy = scale
    return [{"x": float(pt[0][0]) * sx, "y": float(pt[0][1]) * sy} for pt in simplified]
...
def mask_to_all_polygons(
    ...
    ``scale`` = (sx, sy) multiplies every output coordinate + the bounding box, so a
    mask contoured at a REDUCED resolution lands in full-frame pixels. The RDP
    simplification runs at the reduced resolution, where both epsilon terms scale
    linearly with resolution, so the scaled-up polygon carries the SAME visual
    simplification granularity as one traced at full resolution. Default (1, 1) is
    a no-op (byte-identical to the pre-2026-09-13 behavior).
    """
    sx, sy = scale
    if mask.ndim == 3 and mask.shape[0] == 1:
        mask = mask[0]
    ...
    if sx != 1.0 or sy != 1.0:
        # Contoured at reduced resolution - lift every vertex + the bbox into full-frame px.
        all_paths = [[{"x": p["x"] * sx, "y": p["y"] * sy} for p in ring] for ring in all_paths]
        bx0, by0 = global_min_x * sx, global_min_y * sy
        bx1, by1 = global_max_x * sx, global_max_y * sy
    else:
        bx0, by0, bx1, by1 = global_min_x, global_min_y, global_max_x, global_max_y

    return {
        "bounding_box": {"x": bx0, "y": by0, "w": bx1 - bx0, "h": by1 - by0},
...
def mask_to_instance_polygons(
    ...
    for label_id, _area in qualifying:
        ...
        poly_body = mask_to_all_polygons(component, min_area_ratio=min_area_ratio, scale=scale)
