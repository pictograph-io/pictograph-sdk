...
``epsilon = min(0.002 * perimeter, 0.0012 * image_diagonal)``) so auto-annotate
...
def mask_to_polygon(mask: np.ndarray) -> list[dict[str, float]]:
    ...
    epsilon = min(0.002 * cv2.arcLength(largest, True), 0.0012 * image_diagonal)
...
def mask_to_all_polygons(
    ...
    diagonal_eps_cap = 0.0012 * image_diagonal
    ...
    for contour in contours:
        ...
        epsilon = min(0.002 * cv2.arcLength(contour, True), diagonal_eps_cap)
