...
async def image_segment(
    ...
    *,
    ...
    if looks_like_id(image):
        ...
        # bytes. `/{id}/metadata` is the by-id JSON twin that hands back the
        # directory + filename this segment folds together (B333).
        row = await transport.request("GET", f"/api/v1/developer/images/{image}/metadata")
